...
from .models import Certidoes, Cartorio, ClienteTerceiro, Processos, Documento, ClientJob, FinanciamentoVeiculo, ClientEmpresarial,FinanciamentoImovel, ConsultaServicosGeralCPF, ConsultaServicosGeralVeiculo
...
@admin.register(ConsultaServicosGeralCPF)
class NovoClienteModelConsultaCPF(admin.ModelAdmin):
    def get_list_display(self, request):
        return [field.name for field in self.model._meta.fields]

@admin.register(ConsultaServicosGeralVeiculo)
class NovoClienteModelConsultaVeiculo(admin.ModelAdmin):
    def get_list_display(self, request):
        return [field.name for field in self.model._meta.fields]
